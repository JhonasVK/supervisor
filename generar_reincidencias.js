...
async function main() {
  ...
  // Se arma directamente desde los datos ya calculados de esta corrida -- no
  // depende de releer el Excel recien escrito.
  try {
    const tasaParaBase = statsGlobal.total ? statsGlobal.reincidencias / statsGlobal.total : 0;
    await agregarFilaBase('reincidencias', {
      fecha: new Date(),
      total: statsGlobal.total,
      problemas: statsGlobal.reincidencias,
      tasa: +(tasaParaBase * 100).toFixed(2),
      meta: +(META * 100).toFixed(2),
      resultado: tasaParaBase <= META ? 'CUMPLE' : 'NO CUMPLE',
    });
    console.log('Fila agregada a la base historica: Archivos excel\\Base_Reincidencias_COBRA.xlsx');
  } catch (err) {
